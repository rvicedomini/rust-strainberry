...
namespace spoa_rs {
    ...
    unique_ptr<std::string> generate_consensus_with_coverage(unique_ptr<spoa::Graph> const& graph,
                                                             std::unique_ptr<std::vector<uint32_t>> const& summary) {
        return std::make_unique<std::string>(graph->GenerateConsensus(summary.get()));
    }
