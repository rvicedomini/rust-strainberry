...
// pub fn build_read_index(fastx_path: &Path) -> Result<HashMap<String,usize>> {

//     let mut reader = needletail::parse_fastx_file(&fastx_path)?;
...
//         if read_index.insert(name.to_string(), read_index.len()).is_some() {
//             bail!("duplicate read identifier: {name}");
//         }
//     }
